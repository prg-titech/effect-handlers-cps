...
mutual
def Even.toNat : (n : Even) → ValTy
  | .zero => .bool
  | .succ i => .fn .bool (i.toNat, .nil)
-- termination_by n => sizeOf n
def Odd.toNat : (n : Odd) → ValTy
  | .succ i => .fn .bool (i.toNat, .nil)
-- termination_by n => sizeOf n
end

example : (Odd.succ (.zero)).toNat = .fn .bool (.bool, .nil) := rfl
example : (Even.succ (Odd.succ (.zero))).toNat = .fn .bool (.fn .bool (.bool, .nil), .nil) := rfl
