...
def ValTy.trans: ValTy → SystemF.Ty .nil *
  ...
  | .hn C D =>
    have : (
      match C with
      | ⟨A, ⟨E⟩⟩ => sizeOf A + sizeOf E) < 1 + sizeOf C + sizeOf D := by {
        cases C
        case mk A E =>
        cases E
        simp_arith
...
-- @[reducible]
-- def CmpTy.trans: CmpTy → SystemF.Ty Φ *
--   | ⟨A, ⟨E⟩⟩ =>
--     .pi .type (.fn (HandlerTy.trans ⟨A, ⟨E⟩⟩ (.var .zero)) (.var .zero))
--   termination_by c => sizeOf c
@[reducible]
def CmpTy.trans: CmpTy → SystemF.Ty .nil *
  | ⟨A, ⟨E⟩⟩ =>
    .pi .type ((HandlerTy.trans ⟨A, ⟨E⟩⟩ (.var .zero)) ⇒ (.var .zero))
  termination_by c => sizeOf c
@[reducible]
def HandlerTy.trans: CmpTy → SystemF.Ty Φ * → SystemF.Ty Φ *
  | ⟨A, ⟨E⟩⟩, B =>
    ...
    .pair (.fn A.trans{{ρ'}}ᵣ B) (Signature.trans E B)
  termination_by c =>
    match c with
    | ⟨A, ⟨E⟩⟩ => sizeOf A + sizeOf E
@[reducible]
...
def ret_cl : Δ ⊢ ⟦⟨A, ⟨E⟩⟩⟧⇛B → Δ ⊢ SystemF.Ty.fn ⟦A⟧v{{ρ'}}ᵣ B
  | t => by
...
def op_cl : Δ ⊢ ⟦⟨A, ⟨E⟩⟩⟧⇛B → E ∋ ⟨A', B'⟩ → Δ ⊢ .fn ⟦A'⟧v{{ρ'}}ᵣ (.fn (.fn ⟦B'⟧v{{ρ'}}ᵣ B) B)
  | t, i => by
    simp [HandlerTy.trans] at t
    rw [SystemF.Ren'.nil_lemma ρ']
...
def Val.trans : Γ ⊢v A →  ⟦Γ⟧v ⊢ ⟦A⟧v
  ...
  | .handler h (C:=C) (C':=C') =>
    ...
    this ▸ h.trans
termination_by v => sizeOf v
def Cmp.trans: Γ ⊢c ⟨A, ⟨E⟩⟩ → (ρ : SystemF.Ren Δ ⟦Γ⟧v ρ') → Δ ⊢ (⟦⟨A, ⟨E⟩⟩⟧⇛B) → Δ ⊢ B
  | .return v, ρ, h =>
    let v := v.trans
