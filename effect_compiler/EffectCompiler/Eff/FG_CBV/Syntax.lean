...
def CmpTy.val : CmpTy → ValTy
  | ⟨A, _⟩ => A
def CmpTy.eff : CmpTy → Context OpTy
  | ⟨_, ⟨E⟩⟩ => E

mutual
def Val.size : Γ ⊢v A → Nat
  | .var i => 1
  -- | .bool b => 1
  | .false => 1
  | .true => 1
