...
def ValTy.trans: ValTy → SystemF.Ty .nil *
  ...
  | .hn ((A, E), D) =>
    -- have : sizeOf A + sizeOf E < 1 + (1 + sizeOf (A, E) + sizeOf D) := by {
    --   simp_arith
    -- }
...
def HandlerTy.trans: CmpTy → SystemF.Ty Φ * → SystemF.Ty Φ * :=
  ...
  .pair (.fn C.1.trans{{ρ'}}ᵣ' B) (Signature.trans C.2 B)
...
def Val.trans : Γ ⊢v A →  ⟦Γ⟧v ⊢ ⟦A⟧v
  ...
  | .lam A e (C:=C) =>
    ...
    let ρ : SystemF.Ren (⟦Γ; A⟧v;* *; (⟦C⟧⇛(.var .zero))) ⟦Γ; A⟧v ρ' := by  {
      intro T i
      apply SystemF.Con.Member.succ
      apply SystemF.Con.Member.succ'
      assumption
    }
    let this : (⟦A⟧v ⇒ SystemF.Ty.pi .type (⟦C⟧⇛(SystemF.Ty.var .zero) ⇒ SystemF.Ty.var .zero)) = ⟦A ⇒ C⟧v := by {
      simp [ValTy.trans, CmpTy.trans]
    }
    this ▸
    .lam ⟦A⟧v (.Lam .type (.lam (⟦C⟧⇛(.var .zero)) (Cmp.trans e ρ (.var .zero))))
  | .handler (A:=A) (E:=E) ret op (C:=C) =>
    ...
    let ρ : SystemF.Ren (⟦Γ⟧v; ⟦A⟧v;* *; ⟦C⟧⇛(SystemF.Ty.var .zero)) ⟦Γ; A⟧v SystemF.Ren'.wk :=
      fun K i =>
      SystemF.Con.Member.succ <|
      SystemF.Con.Member.succ' i
    this ▸ .pair
    (
      .lam ⟦A⟧v <| .Lam .type <| .lam _ <| Cmp.trans ret ρ (.var .zero)
    )
    (OpClauses.trans op)
termination_by v => sizeOf v
def Cmp.trans: Γ ⊢c C → (ρ : SystemF.Ren Δ ⟦Γ⟧v ρ') → Δ ⊢ (⟦C⟧⇛B) → Δ ⊢ B
  | .return (E:=E) v, ρ, h => .app (ret_cl h) (v.trans.ren ρ)
  ...
  | .app (A:=A') v₁ v₂, ρ, h => by
    let this : ⟦A' ⇒ C⟧v = (⟦A'⟧v ⇒ ⟦C⟧c) := by simp [ValTy.trans]
    let v₁ := this ▸ v₁.trans
    let v₁ := v₁.ren ρ
    let v₂ := v₂.trans
    let v₂ := v₂.ren ρ
...
def OpClauses.trans : OpClauses Γ E C → ⟦Γ⟧v ⊢ Signature.trans E ⟦C⟧c
  ...
  | .cons (A:=A') (B:=B) (E:=E') S c =>
    ...
    let ρ' := SystemF.Ren'.wk
    let ρ : SystemF.Ren _ _ ρ' := by {
      intro K i
      apply SystemF.Con.Member.succ
      apply SystemF.Con.Member.succ'
      rw [←SystemF.Ren'.nil_lemma (SystemF.Ren'.id _), SystemF.Ren'.id_rfl, SystemF.Ren'.id_rfl]
      rw [←Context.trans, ←ValTy.trans, ←Context.trans]
      assumption
    }
    this ▸ .pair (.lam _ (.lam _ (.Lam _ (.lam _ (Cmp.trans c ρ (.var .zero)))))) S.trans
termination_by S => sizeOf S
end
