...
def Term.rename : Δ ⊢ A → Renaming Γ Δ → Γ ⊢ A
  ...
  | .inl a, ρ => .inl (rename a ρ)
  | .inr b, ρ => .inr (rename b ρ)
  | .case m a b, ρ => .case (rename m ρ) (rename a (ρ.extend _)) (rename b (ρ.extend _))
...
def Renaming.eq_id : (t : Γ ⊢ A) → t{{𝟙ᵣΓ}}ᵣ = t
  ...
  | .inl a => congrArg Term.inl (eq_id a)
  | .inr b => congrArg Term.inr (eq_id b)
  | .case m a b => by
    rename_i B C
    simp[Term.rename]
    constructor
    exact eq_id _
    constructor
    have := eq_id_extend Γ B
    simp at this
    rw [this]
    exact eq_id _
    have := eq_id_extend Γ C
    simp at this
    rw [this]
    exact eq_id _
...
def Renaming.ren_comp : (ρ : Renaming Γ Δ) → (ρ' : Renaming Δ Θ) → (t : Θ ⊢ A) → t{{comp ρ ρ'}}ᵣ = t{{ρ'}}ᵣ{{ρ}}ᵣ
  ...
  | ρ, ρ', .inl a => congrArg Term.inl (ren_comp ρ ρ' a)
  | ρ, ρ', .inr b => congrArg Term.inr (ren_comp ρ ρ' b)
  | ρ, ρ', .case m a b => by
    rename_i B C
    simp [Term.rename]
    constructor
    exact ren_comp _ _ _
    constructor
    have := extend_resp_comp ρ ρ' B
    simp at this
    rw [this]
    exact ren_comp _ _ _
    have := extend_resp_comp ρ ρ' C
    simp at this
    rw [this]
    exact ren_comp _ _ _
...
def Term.subst : Δ ⊢ A → Subst Γ Δ → Γ ⊢ A
  ...
  | .inl a, σ => .inl (subst a σ)
  | .inr b, σ => .inr (subst b σ)
  | .case (A:=A) (B:=B) m a b, σ => .case (subst m σ) (subst a (σ ⁺⁺ A)) (subst b (σ ⁺⁺ B))
...
def eq_lift : (t : Δ ⊢ A) → (σ : Renaming Γ Δ) → t{{σ}}ₛ = t{{σ}}ᵣ
  ...
  | .inl a, σ => congrArg Term.inl (eq_lift a σ)
  | .inr b, σ => congrArg Term.inr (eq_lift b σ)
  | .case m a b, σ => by
    rename_i B C
    simp [Term.subst]
    congr
    exact eq_lift _ _
    rw [extend_lift_commute]
    exact eq_lift _ _
    rw [extend_lift_commute]
    exact eq_lift _ _
...
def «{{comp_sub_ren}}ₛ» : (σ : Subst Γ Δ) → (ρ : Renaming Δ Θ) → (t : Θ ⊢ A) → t{{comp_sub_ren σ ρ}}ₛ = t{{ρ}}ᵣ{{σ}}ₛ
  ...
  | σ, ρ, .inl a => congrArg Term.inl («{{comp_sub_ren}}ₛ» σ ρ a)
  | σ, ρ, .inr b => congrArg Term.inr («{{comp_sub_ren}}ₛ» σ ρ b)
  | σ, ρ, .case m a b => by
    rename_i B C
    simp [Term.subst]
    constructor
    exact «{{comp_sub_ren}}ₛ» _ _ _
    constructor
    rw [ext_resp_comp_sub_ren]
    exact «{{comp_sub_ren}}ₛ» _ _ _
    rw [ext_resp_comp_sub_ren]
    exact «{{comp_sub_ren}}ₛ» _ _ _
...
def «{{comp_ren_sub}}ₛ» : (ρ : Renaming Γ Δ) → (σ : Subst Δ Θ) → (t : Θ ⊢ A) → t{{comp_ren_sub ρ σ}}ₛ = t{{σ}}ₛ{{ρ}}ᵣ
  ...
  | σ, ρ, .inl a => congrArg Term.inl («{{comp_ren_sub}}ₛ» σ ρ a)
  | σ, ρ, .inr b => congrArg Term.inr («{{comp_ren_sub}}ₛ» σ ρ b)
  | σ, ρ, .case m a b => by
    rename_i B C
    simp [Term.subst]
    congr
    exact  «{{comp_ren_sub}}ₛ» _ _ _
    rw [ext_resp_comp_ren_sub]
    exact  «{{comp_ren_sub}}ₛ» _ _ _
    rw [ext_resp_comp_ren_sub]
    exact  «{{comp_ren_sub}}ₛ» _ _ _
...
def Subst.comp_refl : (t : Γ ⊢ A) → t{{comp σ σ'}}ₛ = t{{σ'}}ₛ{{σ}}ₛ
  ...
  | .inl a => congrArg Term.inl (comp_refl a)
  | .inr b => congrArg Term.inr (comp_refl b)
  | .case m a b => by
    rename_i B C
    simp [Term.subst]
    constructor
...
instance STLC_Sum : Category (Context Ty) where
  Hom := Subst
  id _ := 𝟙ₛ_
  comp := Subst.comp
