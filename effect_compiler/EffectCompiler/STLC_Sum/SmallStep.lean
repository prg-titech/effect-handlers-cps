...
inductive Normal : Γ ⊢ A → Type
  ...
  | inl     : Normal a → Normal (.inl a)
  | inr     : Normal b → Normal (.inr b)
...
inductive Neutral : Γ ⊢ A → Type
  ...
  | case    : Neutral m → Normal a → Normal b → Neutral (.case m a b)
end
...
inductive SmallStep : Γ ⊢ A → Γ ⊢ A → Prop
  ...
  | ξ_INL         : SmallStep a a' → SmallStep (.inl a) (.inl a')
  | ξ_INR         : SmallStep b b' → SmallStep (.inr b) (.inr b')
  | ξ_CASE₁       : SmallStep m m' → SmallStep (.case m a b) (.case m' a b)
  | ξ_CASE₂       : SmallStep a a' → SmallStep (.case m a b) (.case m a' b)
  | ξ_CASE₃       : SmallStep b b' → SmallStep (.case m a b) (.case m a b')
  | β_P₁          : SmallStep (.p₁ (.pair a b)) a
  | β_P₂          : SmallStep (.p₂ (.pair a b)) b
  | β_APP         : SmallStep ((ƛ _ => e) @ x) e[[x]]₀
  | β_CASE₁       : SmallStep (.case (.inl x) a b) a[[x]]₀
  | β_CASE₂       : SmallStep (.case (.inr x) a b) b[[x]]₀
...
theorem Normal.not_progress {Γ} {A} {t : Γ ⊢ A} : (Normal t) → ∀ t', ¬(t ⟶ₛ t')
  ...
  | .inl hva =>
    fun t ht =>
    match ht with
    | .ξ_INL haa' => Normal.not_progress hva _ haa'
  | .inr hvb =>
    fun t ht =>
    match ht with
    | .ξ_INR hbb' => Normal.not_progress hvb _ hbb'
...
theorem Neutral.not_progress {Γ} {A} {t : Γ ⊢ A} : (Neutral t) → ∀ t', ¬(t ⟶ₛ t')
  ...
  | .case hm ha hb =>
    fun t ht =>
    match ht with
    | .ξ_CASE₁ hmm' => Neutral.not_progress hm _ hmm'
    | .ξ_CASE₂ haa' => Normal.not_progress ha _ haa'
    | .ξ_CASE₃ hbb' => Normal.not_progress hb _ hbb'
    | .β_CASE₁ => by contradiction
    | .β_CASE₂ => by contradiction
end
-- termination_by
...
inductive Parallel : Γ ⊢ A → Γ ⊢ A → Prop where
  ...
  | ξ_inl : Parallel a a' → Parallel (.inl a) (.inl a')
  | ξ_inr : Parallel b b' → Parallel (.inr b) (.inr b')
  | ξ_case : Parallel m m' → Parallel a a' → Parallel b b' → Parallel (.case m a b) (.case m' a' b')
  | ξβ_case₁ : Parallel x x' → Parallel a a' → Parallel (.case (.inl x) a b) a'[[x']]₀
  | ξβ_case₂ : Parallel x x' → Parallel b b' → Parallel (.case (.inr x) a b) b'[[x']]₀
infix: 30 " ⟹ " => Parallel
...
def Parallel.refl : (t : Γ ⊢ A) → t ⟹ t
  ...
  | .inl a => .ξ_inl (refl a)
  | .inr b => .ξ_inr (refl b)
  | .case m a b => .ξ_case (refl m) (refl a) (refl b)
...
def «⟹of⟶ₛ» : t ⟶ₛ t' → t ⟹ t'
  ...
  | .ξ_INL haa => .ξ_inl («⟹of⟶ₛ» haa)
  | .ξ_INR hbb => .ξ_inr («⟹of⟶ₛ» hbb)
  | .ξ_CASE₁ hmm' => .ξ_case («⟹of⟶ₛ» hmm') (Parallel.refl _) (Parallel.refl _)
  | .ξ_CASE₂ haa' => .ξ_case (Parallel.refl _) («⟹of⟶ₛ» haa') (Parallel.refl _)
  | .ξ_CASE₃ hbb' => .ξ_case (Parallel.refl _) (Parallel.refl _) («⟹of⟶ₛ» hbb')
  | .β_CASE₁ => .ξβ_case₁ (Parallel.refl _) (Parallel.refl _)
  | .β_CASE₂ => .ξβ_case₂ (Parallel.refl _) (Parallel.refl _)
...
def MultiStep.ξ_inl : a ⟶* a' → .inl (B:=B) a ⟶* .inl  a' :=
  fun haa' => ReflTransGen.lift Term.inl (fun _ _ => .ξ_INL) haa'
def MultiStep.ξ_inr : b ⟶* b' → .inr (A:=A) b ⟶* .inr  b' :=
  fun hbb' => ReflTransGen.lift Term.inr (fun _ _ => .ξ_INR) hbb'
def MultiStep.ξ_case₁ : m ⟶* m' → .case m a b ⟶* .case m' a b :=
  fun hmm' => ReflTransGen.lift (Term.case · _ _) (fun _ _ => .ξ_CASE₁) hmm'
def MultiStep.ξ_case₂ : a ⟶* a' → .case m a b ⟶* .case m a' b :=
  fun haa' => ReflTransGen.lift (Term.case _ · _) (fun _ _ => .ξ_CASE₂) haa'
def MultiStep.ξ_case₃ : b ⟶* b' → .case m a b ⟶* .case m a b' :=
  fun hbb' => ReflTransGen.lift (Term.case _ _ ·) (fun _ _ => .ξ_CASE₃) hbb'
def MultiStep.ξ_case : m ⟶* m' → a ⟶* a' → b ⟶* b' → .case m a b ⟶* .case m' a' b' :=
  fun hmm' haa' hbb' => .trans (.trans (MultiStep.ξ_case₁ hmm') (MultiStep.ξ_case₂ haa')) (MultiStep.ξ_case₃ hbb')
def «⟶*of⟹» {t t' : Γ ⊢ A} : t ⟹ t' → t ⟶* t'
  ...
  | .ξ_inl haa' => MultiStep.ξ_inl («⟶*of⟹» haa')
  | .ξ_inr hbb' => MultiStep.ξ_inr («⟶*of⟹» hbb')
  | .ξ_case hmm' haa' hbb' => MultiStep.ξ_case («⟶*of⟹» hmm') («⟶*of⟹» haa') («⟶*of⟹» hbb')
  | .ξβ_case₁ hxx' haa' => .tail (MultiStep.ξ_case (MultiStep.ξ_inl («⟶*of⟹» hxx')) («⟶*of⟹» haa') .refl) .β_CASE₁
  | .ξβ_case₂ hxx' hbb' => .tail (MultiStep.ξ_case (MultiStep.ξ_inr («⟶*of⟹» hxx')) .refl («⟶*of⟹» hbb')) .β_CASE₂
...
def Term.cd : Γ ⊢ A → Γ ⊢ A
  ...
  | .inl a => .inl a.cd
  | .inr b => .inr b.cd
  | .case (.inl x) a b => a.cd[[x.cd]]₀
  | .case (.inr x) a b => b.cd[[x.cd]]₀
  | .case m a b => .case m.cd a.cd b.cd
-- termination_by
--   Term.cd t => t.size
postfix : max "⋆" => Term.cd
...
def Parallel.ξρ : t ⟹ t' → t{{ρ}}ᵣ ⟹ t'{{ρ}}ᵣ
  ...
  | .ξβ_app (B:=A) hee' (e:=e) (e':=e') hxx' (x:=x) (x':=x') => by
    simp [STLC_Sum.Term.rename]
    rw [Parallel.ξρ_lemma]
    exact .ξβ_app (Parallel.ξρ hee') (Parallel.ξρ hxx')
  | .ξ_inl haa' => .ξ_inl (Parallel.ξρ haa')
  | .ξ_inr hbb' => .ξ_inr (Parallel.ξρ hbb')
  | .ξ_case hmm' haa' hbb' => .ξ_case (Parallel.ξρ hmm') (Parallel.ξρ haa') (Parallel.ξρ hbb')
  | .ξβ_case₁ hxx' haa' => by
    simp [STLC_Sum.Term.rename]
    rw [Parallel.ξρ_lemma]
    apply Parallel.ξβ_case₁
    all_goals (apply Parallel.ξρ); assumption
  | .ξβ_case₂ hxx' hbb' => by
    simp [STLC_Sum.Term.rename]
    rw [Parallel.ξρ_lemma]
    apply Parallel.ξβ_case₂
...
def Parallel.ξσ {t t' : Δ ⊢ A} {σ σ' : Subst Γ Δ} : t ⟹ t' → (∀ B : Ty, ∀ x : Δ ∋ B, σ B x ⟹ σ' B x) → t{{σ}}ₛ ⟹ t'{{σ'}}ₛ
  ...
  | .ξ_inl haa', h => .ξ_inl (ξσ haa' h)
  | .ξ_inr hbb', h => .ξ_inr (ξσ hbb' h)
  | .ξ_case hmm' haa' hbb', h => .ξ_case (ξσ hmm' h) (ξσ haa' (ξσ_lemma₁ h)) (ξσ hbb' (ξσ_lemma₁ h))
  | .ξβ_case₁ hxx' haa', h => by
    simp [STLC_Sum.Term.subst]
    rw [Parallel.ξσ_lemma₂ σ']
    apply Parallel.ξβ_case₁
    apply ξσ
    assumption
    assumption
    apply ξσ
    assumption
    apply Parallel.ξσ_lemma₁
    assumption
  | .ξβ_case₂ hxx' hbb', h => by
    simp [STLC_Sum.Term.subst]
    rw [Parallel.ξσ_lemma₂ σ']
    apply Parallel.ξβ_case₂
    apply ξσ
...
def SmallStep.ξρ : t ⟶ₛ t' → t{{ρ}}ᵣ ⟶ₛ t'{{ρ}}ᵣ
  ...
  | .β_APP => by
    ...
    rw [Parallel.ξρ_lemma]
    exact .β_APP
  | .ξ_INL haa' => .ξ_INL (ξρ haa')
  | .ξ_INR hbb' => .ξ_INR (ξρ hbb')
  | .ξ_CASE₁ hmm' => .ξ_CASE₁ (ξρ hmm')
  | .ξ_CASE₂ haa' => .ξ_CASE₂ (ξρ haa')
  | .ξ_CASE₃ hbb' => .ξ_CASE₃ (ξρ hbb')
  | .β_CASE₁ => by
    simp [STLC_Sum.Term.rename]
    rw [Parallel.ξρ_lemma]
    exact .β_CASE₁
...
def SmallStep.ξσ : t ⟶ₛ t' → t{{σ}}ₛ ⟶ₛ t'{{σ}}ₛ
  ...
  | .β_APP => by
    ...
    exact .β_APP
  | .ξ_INL haa' => .ξ_INL (ξσ haa')
  | .ξ_INR hbb' => .ξ_INR (ξσ hbb')
  | .ξ_CASE₁ hmm' => .ξ_CASE₁ (ξσ hmm')
  | .ξ_CASE₂ haa' => .ξ_CASE₂ (ξσ haa')
  | .ξ_CASE₃ hbb' => .ξ_CASE₃ (ξσ hbb')
  | .β_CASE₁ => by
    simp [STLC_Sum.Term.subst]
    rw [Parallel.ξσ_lemma₂]
    exact .β_CASE₁
...
def Parallel.confluence {t t' : Γ ⊢ A} : t ⟹ t' → t' ⟹ t⋆ := by
  ...
  | ξ_inr hbb' ihb =>
    ...
    constructor
    assumption
  | ξ_case hmm' haa' hbb' ihm iha ihb =>
    rename_i m m' a a' b b'
    cases m
    any_goals simp_all; constructor; assumption; assumption; assumption
    {
      simp_all
      cases m'
      any_goals contradiction
  ...
  | ξβ_case₁ hxx' haa' ihx iha =>
    rename_i x x' a a'
    simp
    apply Parallel.ξσ iha
    intro B y
    cases y with
    | zero => trivial
    | succ y => constructor
...
def SmallStep.ξσ' {σ σ' : Subst Γ Δ} : {t : Δ ⊢ A} → (∀ B : Ty, ∀ x : Δ ∋ B, σ B x ⟶* σ' B x) → t{{σ}}ₛ ⟶* t{{σ'}}ₛ
  ...
  | .inl _, h => MultiStep.ξ_inl (ξσ' h)
  | .inr _, h => MultiStep.ξ_inr (ξσ' h)
  | .case _ _ _, h => MultiStep.ξ_case (ξσ' h) (ξσ' (SmallStep.ξσ_lemma h)) (ξσ' (SmallStep.ξσ_lemma h))
...
def MultiStep.ξσ' {σ σ' : Subst Γ Δ} : {t: Δ ⊢ A} → (∀ B : Ty, ∀ x : Δ ∋ B, σ B x ⟶* σ' B x) → t{{σ}}ₛ ⟶* t{{σ'}}ₛ
  ...
  | .inl a, h => MultiStep.ξ_inl (ξσ' h)
  | .inr a, h => MultiStep.ξ_inr (ξσ' h)
  | .case m a b, h => MultiStep.ξ_case (ξσ' h) (ξσ' (ξσ_lemma₁ h)) (ξσ' (ξσ_lemma₁ h))
...
def renaming_neutral : {t : Γ ⊢ A} → Neutral t → Neutral t{{ρ}}ᵣ
  ...
  | .case m a b, .case hnem hna hnb => .case (renaming_neutral hnem) (renaming_normal hna) (renaming_normal hnb)
def renaming_normal : {t : Γ ⊢ A} → Normal t → Normal t{{ρ}}ᵣ
  ...
  | .inl a, .inl hna => .inl (renaming_normal hna)
  | .inr b, .inr hnb => .inr (renaming_normal hnb)
