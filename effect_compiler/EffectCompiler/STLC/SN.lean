...
import EffectCompiler.STLC.TypeSafety
...
def pair : sn (.pair a b) → sn a ∧ sn b := by
  generalize h : STLC.Term.pair a b = t
  intro hsn_t
...
def lam : sn (.lam B e) → sn e := by
  generalize h : STLC.Term.lam B e = t
  intro hsn
  induction hsn generalizing e
  case intro t h ih =>
  constructor
...
def R_σ : (t : Δ ⊢ A) → Γ ⊨ σ → R A t{{σ}}ₛ
  ...
  | .lam B e, h =>
    ...
    have h : ∀ (Δ : Context Ty) (ρ : Renaming Δ Γ) (x : Δ ⊢ B), R B x → R _ ((STLC.Term.lam B e){{σ}}ₛ{{ρ}}ᵣ @ x) :=
      fun Δ ρ x hr_x => by
