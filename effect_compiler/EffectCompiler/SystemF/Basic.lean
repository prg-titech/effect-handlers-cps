...
def Renameₜ.id : ∀ {Φ}, Renameₜ Φ Φ := _root_.id
notation : max "𝟙ᵣₜ" => Renameₜ.id
...
def Renameₜ.id_rfl : {A : Ty Φ} → A{{𝟙ᵣₜ}}ᵣₜ = A
  | .var i => rfl
  | .unit => rfl
  | .bool => rfl
  | .fn A B => by
...
def Substₜ.id : Substₜ Φ Φ := (.var ·)
notation : max "𝟙ₛₜ" => Substₜ.id
...
def Substₜ.id_extend : (Substₜ.id (Φ:=Φ)).extend = Substₜ.id :=
  funext fun i =>
  match i with
  | .zero => rfl
  | .succ _ => rfl

def Substₜ.subs_id : {t : Ty Φ} → t{{Substₜ.id}}ₛₜ = t
  | .var _ => rfl
  | .unit => rfl
  | .bool => rfl
...
def Rename.id : (Γ : Con Φ) → Rename Φ Φ Γ Γ Renameₜ.id
  | _, _ => Renameₜ.id_rfl ▸ _root_.id
prefix : max "𝟙ᵣ" => Rename.id

def Rename.nil : (Γ : Con Φ) → Rename Φ .nil Γ .nil Renameₜ.nil := sorry
...
def Term.substₜ₀ : Φ;* / Γ.wk ⊢ A → (B : Ty Φ) → Φ / Γ ⊢ A[[B]]ₛₜ :=
  fun t B =>
  have : ∀ {Φ} {Γ: Con Φ} {B}, Γ.wk{{𝟙ₛₜ; B}}ₛₜ = Γ := by {
    intro Φ Γ B
    induction Γ with
    | nil => trivial
    | cons Γ C ih =>
...
def Subst.extend' : Subst Ψ Φ Δ Γ ρₜ → Subst (Ψ;*) (Φ;*) Δ.wk Γ.wk ρₜ.extend :=
  ...
  | .zero =>
    ...
    this ▸ (σ .zero).renameₜ .succ
  | .succ i =>
    let σ' : Subst _ _ _ _ _ := σ ∘ .succ
    σ'.extend' i
...
def Subst.id : Subst Φ Φ Γ Γ (Renameₜ.id) :=
  Renameₜ.id_rfl ▸ (.var ·)
notation : max "𝟙ₛ" => Subst.id

def Term.subst₀ : Φ / Γ; A ⊢ B → Φ / Γ ⊢ A → Φ / Γ ⊢ B :=
  fun t s =>
  have : ∀ {A}, Φ / Γ ⊢ A = Φ / Γ ⊢ A{{𝟙ᵣₜ}}ᵣₜ := by {
    intro A
    congr
    exact Renameₜ.id_rfl.symm
  }
  let σ : Subst _ _ _ _ _ := 𝟙ₛ; (this ▸ s)
  this ▸ t{{σ}}ₛ
...
def Subst.extend' : Subst Φ Δ Γ → Subst (Φ;*) Δ.wk Γ.wk :=
  ...
  | .cons Γ A =>
  match i with
  | .zero => (σ .zero).renameₜ .succ
  | .succ i =>
    let σ' : Subst _ _ _ := σ ∘ .succ
    σ'.extend' i
...
def Typing.rename : {t : Φ / Γ ⊢} → Typing t A → (ρ : Rename Ψ Φ Δ Γ ρₜ) → Typing t{{ρ}}ᵣ A{{ρₜ}}ᵣₜ := sorry
