...
def OpTy.trans : (trans : ValTy → SystemF.ty Varₜ) → OpTy → SystemF.ty Varₜ → SystemF.ty Varₜ
  | trans, (A, B), C => SystemF.ty.fn (trans A) (.fn (.fn (trans B) C) C)

def Signature.trans : (trans : ValTy → SystemF.ty Varₜ) → Signature → SystemF.ty Varₜ → SystemF.ty Varₜ
  | trans, E, B => SystemF.ty.record (fun i => OpTy.trans trans (E.get i) B)

def HandlerTy.trans: (trans : ValTy → SystemF.ty Varₜ) → CmpTy → SystemF.ty Varₜ → SystemF.ty Varₜ :=
  fun trans ⟨A, E⟩ B =>
  .pair (.fn (trans A) B) (Signature.trans trans E B)

def CmpTy.trans: (trans : ValTy → SystemF.ty Varₜ) → CmpTy → SystemF.ty Varₜ :=
  fun trans C => .pi fun X => ((HandlerTy.trans trans C (SystemF.ty.fn (.var X) (.var X))))

def ValTy.trans': ValTy → SystemF.ty Varₜ
  | .bool => .bool
  | .fn A C =>
    .fn A.trans' (CmpTy.trans ValTy.trans' C)
  | .hn C D =>
    HandlerTy.trans ValTy.trans' C (CmpTy.trans ValTy.trans' D)
decreasing_by
  sorry
  sorry
  sorry
