...
data tysch (τ : Set) : Set where
    `_ : ty τ → tysch τ
    Π_ : (τ → tysch τ) → tysch τ

_⇒'_ : ∀ {τ : Set} → tysch τ → tysch τ → tysch τ
(` A) ⇒' (` B) = ` (A ⇒ B)
(Π f) ⇒' (` B) = Π (λ x → (f x) ⇒' (` B))
σ ⇒' (Π f) = Π (λ x → σ ⇒' (f x))
...
data _[_]ty↦_ {τ : Set} : (τ → ty τ) → ty τ → ty τ → Set where
    var= : ∀ (A : ty τ) → (λ x → var x) [ A ]ty↦ A
    var≠ : ∀ (A : ty τ) (x : τ) → (λ _ → var x) [ A ]ty↦ (var x)
    ...
    _⇒_ : ∀ {f₁ f₂} {A A₁ A₂} →
        ...
        ---------------
        (λ a → f₁ a ⇒ f₂ a) [ A ]ty↦ (A₁ ⇒ A₂)
data _[_]tysch↦_ {τ : Set} : (τ → tysch τ) → ty τ → tysch τ → Set where
    `_ : ∀ {A : ty τ} {B : τ → ty τ} {B' : ty τ} → B [ A ]ty↦ B' → (λ x → ` (B x)) [ A ]tysch↦ (` B')
    Π_ : ∀ {f : τ → τ → tysch τ} {f'} {A } →
        (∀ a → (λ b → f b a) [ A ]tysch↦ f' a) → 
        ---------------------------------
        (λ a → Π (f a)) [ A ]tysch↦ (Π f')

data _≻_ {τ : Set} : tysch τ → ty τ → Set where
    `_ : (t : ty τ) → (` t) ≻ t
    Π_ :
        ∀ {σ : τ → tysch τ} {σ' : tysch τ} {t t' : ty τ} →
        σ [ t' ]tysch↦ σ' →
        σ' ≻ t →
        (Π σ) ≻ t

data term (τ : Set) (ν : tysch τ → Set) : ty τ → Set where
    var : ∀ {σ : tysch τ} → (x : ν σ) → {A : ty τ} → σ ≻ A → term τ ν A
    unit : term τ ν unit
    true : term τ ν bool
    false : term τ ν bool
    ...
    Let_In_ : ∀ {σ : tysch τ} {A : ty τ} → ({B : ty τ} → σ ≻ B → term τ ν B) → (ν σ → term τ ν A) → term τ ν A
...
{-# TERMINATING #-}
[_]vty : ∀ {τ} → ValTy → tysch τ
[_]cty : ∀ {τ} → CmpTy → tysch τ
[_]⇛_ : ∀ {τ} → CmpTy → ty τ → tysch τ
[_]eff_ : ∀ {τ} → Eff → ty τ → tysch τ
